...
bool check_arg(int * argc, char * argv[], const char * s);
...
int main(int argc, char *argv[])
{
    ...
    if (summary || trace) {
	if (PLA != nullptr) PLA_summary(PLA);
	if (PLA1 != nullptr) PLA_summary(PLA1);
    }

/*
...
/******************** Espresso operations ********************/
	...
	do {
	    ...
	    if (print_solution) {
            fprint_pla(std::cout, PLA, out_type);
            std::cout.flush();
	    }
...
/******************** Simple cover operations ********************/
    ...
    /* Output the solution */
    if (print_solution) {
        EXECUTE(fprint_pla(std::cout, PLA, out_type), WRITE_TIME, PLA->F, cost);
    }
...
void getPLA(int opt, int argc, char * argv[], int option, pPLA * PLA, int out_type)
{
    ...
    (*PLA)->filename = strcpy(new char[strlen(fname)+1], fname);
