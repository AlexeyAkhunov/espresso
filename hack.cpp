...
/*	    } else if (p2->variable != cube.output) {
	...
	newF = new_cover(100);
	newD = new_cover(100);
	find_inputs(nullptr, PLA, p1->symbolic_list, base, 0,
			    &newF, &newD);
/*
...
void map_symbolic(pPLA PLA)
{
    ...
    new_part_size = new int[num_vars];
...
void symbolic_hack_labels(pPLA PLA, symbolic_t * list, pset compress, int new_size, int old_size, int size_added)
{
    ...
    if ((oldlabel = PLA->label) == nullptr)
	return;
    PLA->label = new char *[new_size];
    ...
    for(p1 = list; p1 != nullptr; p1 = p1->next) {
	p3 = p1->symbolic_label;
	for(i = 0; i < (1 << p1->symbolic_list_length); i++) {
	    if (p3 == nullptr) {
		PLA->label[base+i] = new char[10];
...
void disassemble_fsm(pPLA PLA, int verbose_mode)
{
    ...
    foreach_set(PLA->F, last, p) {
        kiss_print_cube(std::cout, PLA, p, "~1");
    }
}
