...
static void cb_unravel(pcube c, int start, int end, pcube startbase, pcover B1)
{
...
pcover sort_reduce(pcover T)
{
    ...
    foreach_set(T, last, p)
    PUTSIZE(p, ((n - cdist(largest,p)) << 7) + std::min(set_ord(p),127));
...
unsigned int comp_debug)
{
    ...
    numcube = static_cast<int>(CUBELISTSIZE(T));
	...
	*A = A1 = new pcube[numcube+3];
	*B = B1 = new pcube[numcube+3];
	(*A)[0] = set_save(T[0]);
	(*B)[0] = set_save(T[0]);
	A1 = *A + 2;
	B1 = *B + 2;
