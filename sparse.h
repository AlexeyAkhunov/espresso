...
extern sm_matrix *sm_alloc(), *sm_alloc_size(), *sm_dup(sm_matrix * A);
extern void sm_free(sm_matrix * A), sm_delrow(sm_matrix *, int), sm_delcol(sm_matrix *, int), sm_resize(sm_matrix * A, int row, int col);
extern void sm_write(std::ostream& fp, sm_matrix * A), sm_print(), sm_dump(), sm_cleanup();
extern void sm_copy_row(), sm_copy_col();
extern void sm_remove(), sm_remove_element(sm_matrix * A, sm_element * p);
extern sm_element *sm_insert(sm_matrix * A, int row, int col), *sm_find();
extern sm_row *sm_longest_row();
extern sm_col *sm_longest_col();
extern int sm_read(), sm_read_compressed();

extern sm_row *sm_row_alloc(), *sm_row_dup(sm_row *), *sm_row_and();
extern void sm_row_free(sm_row *), sm_row_remove(sm_row * prow, int col), sm_row_print();
extern sm_element *sm_row_insert(sm_row *, int), *sm_row_find(sm_row * prow, int col);
...
extern void sm_col_free(sm_col * pcol), sm_col_remove(), sm_col_print();
extern sm_element *sm_col_insert(sm_col *, int), *sm_col_find(sm_col * pcol, int row);
extern int sm_col_contains(sm_col * p1, sm_col * p2), sm_col_intersects();
