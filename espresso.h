...
#define ptime()		std::chrono::high_resolution_clock::now()
#define print_time(t)	std::to_string((t).count())
...
#define set_new(size)   set_clear(new unsigned int[SET_SIZE(size)], size)
#define set_full(size)  set_fill(new unsigned int[SET_SIZE(size)], size)
#define set_save(r)     set_copy(new unsigned int[SET_SIZE(NELEM(r))], r)
#define set_free(r)     delete(r)
...
#define INLINEset_copy(r, a)\
    {int i_=LOOPCOPY(a); do r[i_]=a[i_]; while (--i_>=0);}
#define INLINEset_clear(r, size)\
    {int i_=LOOPINIT(size); *r=i_; do r[i_] = 0; while (--i_ > 0);}
#define INLINEset_fill(r, size)\
    {int i_=LOOPINIT(size); *r=i_; \
    r[i_]=((unsigned int)(~0))>>(i_*BPI-size); while(--i_>0) r[i_]=~0;}
#define INLINEset_and(r, a, b)\
    {int i_=LOOP(a); PUTLOOP(r,i_);\
...
#define INLINEsetp_implies(a, b, when_false)\
    {int i_=LOOP(a); do if (a[i_]&~b[i_]) break; while (--i_>0);\
    if (i_ != 0) when_false;}
#define INLINEsetp_disjoint(a, b, when_false)\
    {int i_=LOOP(a); do if (a[i_]&b[i_]) break; while (--i_>0);\
    if (i_ != 0) when_false;}
...
#define free_cubelist(T)        delete(T[0]); delete(T);
...
#define EXEC(fct, name, S)\
    {const auto t=ptime();fct;if(trace)print_trace(S,name,ptime()-t);}
#define EXEC_S(fct, name, S)\
    {const auto t=ptime();fct;if(summary)print_trace(S,name,ptime()-t);}
#define EXECUTE(fct,i,S,cost)\
    {const auto t=ptime();fct;totals(t,i,S,&(cost));}
/* lightweight EXECUTE */
#define S_EXECUTE(fct,i)\
    {const auto t=ptime();fct;s_totals(t,i);}

/*
...
extern const char *total_name[TIME_COUNT];    /* basic function names */
extern std::chrono::duration<double> total_time[TIME_COUNT];     /* time spent in basic fcts */
...
/* compl.c */	extern pcover simplify(pcube * T /* T will be disposed of */);
/* compl.c */	extern void simp_comp(pcube * T /* T will be disposed of */, pcover * Tnew, pcover * Tbar);
/* contain.c */	extern int d1_rm_equal(pset *A1 /* array of set pointers */, int (*compare)(const void*, const void*) /* comparison function */);
/* contain.c */	extern int rm2_contain(pset * A1 /* updated in place */, pset * B1 /* unchanged */);
/* contain.c */	extern int rm2_equal(pset * A1, pset * B1 /* updated in place */, pset * E1, int (*compare)(const void*, const void*));
/* contain.c */	extern int rm_contain(pset * A1 /* updated in place */);
/* contain.c */	extern int rm_equal(pset * A1 /* updated in place */, int (*compare)(const void*, const void*));
/* contain.c */	extern int rm_rev_contain(pset * A1 /* updated in place */);
/* contain.c */	extern pset *sf_list(pset_family A);
/* contain.c */	extern pset *sf_sort(pset_family A, int (*compare)(const void*, const void*));
/* contain.c */	extern pset_family d1merge(pset_family A /* disposes of A */, int var);
/* contain.c */	extern pset_family dist_merge(pset_family A /* disposes of A */, pset mask /* defines variables to mask out */);
/* contain.c */	extern pset_family sf_contain(pset_family A /* disposes of A */);
/* contain.c */	extern pset_family sf_dupl(pset_family A /* disposes of A */);
/* contain.c */	extern pset_family sf_ind_contain();
/* contain.c */	extern pset_family sf_ind_unlist(pset * A1, int totcnt, int size, int * row_indices, pset pfirst);
/* contain.c */	extern pset_family sf_merge(pset * A1, pset * B1, pset * E1 /* will be disposed of */, int totcnt, int size);
/* contain.c */	extern pset_family sf_rev_contain(pset_family A /* disposes of A */);
/* contain.c */	extern pset_family sf_union(pset_family A, pset_family B /* disposes of A and B */);
...
/* cvrin.c */	extern int read_pla(std::istream& fp, bool needs_dcset, bool needs_offset, int pla_type, pPLA * PLA_return);
/* cvrin.c */	extern int read_symbolic(std::istream& fp, pPLA PLA, char* word /* scratch string for words */, symbolic_t ** retval);
...
/* cvrin.c */	extern void skip_line(std::istream& fpin, std::ostream& fpout, bool echo);
/* cvrm.c */	extern void foreach_output_function(pPLA PLA, int (*func)(pPLA, int), int (*func1)(pPLA, int));
/* cvrm.c */	extern int cubelist_partition(pcube *T,			/* a list of cubes */
                                              pcube **A, pcube **B,		/* cubelist of partition and remainder */
...
/* cvrm.c */	extern pcover sort_reduce(pcover T);
/* cvrm.c */	extern pcover uncof_output();
/* cvrm.c */	extern pcover unravel(pcover B, int start);
/* cvrm.c */	extern pcover unravel_range(pcover B, int start, int end);
...
/* cvrmisc.c */	extern char *print_cost(pcover F);
/* cvrmisc.c */	extern char *strsav();
/* cvrmisc.c */	extern void copy_cost(pcost s, pcost d);
/* cvrmisc.c */	extern void cover_cost(pcover F, pcost cost);
/* cvrmisc.c */	extern void fatal(const char *);
/* cvrmisc.c */	extern void print_trace(pcover T, const char* name, const std::chrono::duration<double>& time);
/* cvrmisc.c */	extern void size_stamp(pcover T, const char * name);
/* cvrmisc.c */	extern void totals(const std::chrono::time_point<std::chrono::high_resolution_clock>& time, int i, pcover T, pcost cost);
/* cvrout.c */	extern char *fmt_cube(pcube c, const char * out_map, char * s);
...
/* cvrout.c */	extern void kiss_output(std::ostream& fp, pPLA PLA);
/* cvrout.c */	extern void kiss_print_cube(std::ostream& fp, pPLA PLA, pcube p, const char* out_string);
/* cvrout.c */	extern void output_symbolic_constraints(std::ostream& fp, pPLA PLA, int output_symbolic);
/* cvrout.c */	extern void cprint(pcover T);
/* cvrout.c */	extern void debug1_print(pcover T, const char * name, int num);
/* cvrout.c */	extern void debug_print(pcube * T, const char * name, int level);
/* cvrout.c */	extern void eqn_output(pPLA PLA);
/* cvrout.c */	extern void fpr_header(std::ostream& fp, pPLA PLA, int output_type);
/* cvrout.c */	extern void fprint_pla(std::ostream& fp, pPLA PLA, int output_type);
/* cvrout.c */	extern void pls_group(pPLA PLA, std::ostream& fp);
/* cvrout.c */	extern void pls_label(pPLA PLA, std::ostream& fp);
/* cvrout.c */	extern void pls_output(pPLA PLA);
/* cvrout.c */	extern void print_cube(std::ostream& fp, const pcube& c, const char * out_map);
/* cvrout.c */	extern void print_expanded_cube(std::ostream& fp, pcube c, pcube phase);
...
/* essen.c */	extern bool essen_cube(pcover F, pcover D, pcube c);
/* essen.c */	extern pcover cb_consensus(pcover T, pcube c);
/* essen.c */	extern pcover cb_consensus_dist0(pcover R, pcube p, pcube c);
/* essen.c */	extern pcover essential(pcover * Fp, pcover * Dp);
...
/* expand.c */	extern pcover expand(pcover F, pcover R, bool nonsparse  /* expand non-sparse variables only */);
...
/* gasp.c */	extern void expand1_gasp(pcover F,		/* reduced cubes of ON-set */
                                         pcover D,		/* DC-set */
                                         pcover R,		/* OFF-set */
                                         pcover Foriginal,	/* ON-set before reduction (same order as F) */
                                         int c1index,		/* which index of F (or Freduced) to be checked */
                                         pcover *G);
/* getopt.c */	extern int getopt(int argc, char* argv[], const char * optstring);
...
/* hack.c */	extern void form_bitvector(pset p,			/* old cube, looking at binary variables */
...
/* hack.cpp */ extern void disassemble_fsm(pPLA PLA, int verbose_mode);
/* irred.c */	extern bool cube_is_covered(pcube * T, pcube c);
/* irred.c */	extern bool taut_special_cases(pcube * T /* will be disposed if answer is determined */);
/* irred.c */	extern bool tautology(pcube * T /* T will be disposed of */);
...
/* primes.c */	extern pcover primes_consensus(pcube * T /* T will be disposed of */);
...
/* reduce.c */	extern pcover reduce(pcover F, pcover D);
/* reduce.c */	extern pcube reduce_cube(pcube * FD, pcube p);
/* reduce.c */	extern pcube sccc(pcube * T /* T will be disposed of */);
/* reduce.c */	extern pcube sccc_cube();
/* reduce.c */	extern pcube sccc_merge(pcube left, pcube right, pcube cl, pcube cr);
/* set.c */	extern bool set_andp(pset r, pset a, pset b);
/* set.c */	extern bool set_orp();
/* set.c */	extern bool setp_disjoint(pset a, pset b);
/* set.c */	extern bool setp_empty(pset a);
/* set.c */	extern bool setp_equal(pset a, pset b);
/* set.c */	extern bool setp_full();
/* set.c */	extern bool setp_implies(pset a, pset b);
/* set.c */	extern char *pbv1(pset s, int n);
/* set.c */	extern char *ps1(pset a);
/* set.c */	extern int *sf_count(pset_family A);
/* set.c */	extern int *sf_count_restricted(pset_family A, pset r);
/* set.c */	extern int bit_index(unsigned int a);
/* set.c */	extern int set_dist(pset a, pset b);
/* set.c */	extern int set_ord(pset a);
/* set.c */	extern void set_adjcnt(pset a, int * count, int weight);
/* set.c */	extern pset set_and(pset r, pset a, pset b);
/* set.c */	extern pset set_clear(pset, int);
/* set.c */	extern pset set_copy(pset, pset);
/* set.c */	extern pset set_diff(pset r, pset a, pset b);
/* set.c */	extern pset set_fill(pset r, int size);
/* set.c */	extern pset set_merge(pset r, pset a, pset b, pset mask);
...
/* set.c */	extern pset_family sf_compress(pset_family A /* will be freed */, pset c);
...
/* set.c */	extern pset_family sf_delcol(pset_family A, int firstcol, int n);
...
/* set.c */	extern pset_family sf_permute(pset_family A, int * permute, int npermute);
/* set.c */	extern pset_family sf_read();
/* set.c */	extern pset_family sf_save(pset_family A);
/* set.c */	extern pset_family sf_transpose();
/* set.c */	extern void set_write(std::ostream& fp, pset a);
...
/* setc.c */	extern bool ccommon(pcube a, pcube b, pcube cof);
/* setc.c */	extern bool cdist0(pcube a, pcube b);
/* setc.c */	extern bool full_row(pcube p, pcube cof);
/* setc.c */	extern int ascend(const void * v1, const void * v2);
/* setc.c */	extern int cactive(pcube a);
/* setc.c */	extern int cdist(pset a, pset b);
/* setc.c */	extern int cdist01(pset a, pset b);
/* setc.c */	extern int cvolume();
/* setc.c */	extern int d1_order(const void * v1, const void * v2);
/* setc.c */	extern int d1_order_size();
/* setc.c */	extern int desc1(pset a, pset b);
/* setc.c */	extern int descend(const void * v1, const void * v2);
/* setc.c */	extern int lex_order(const void * v1, const void * v2);
/* setc.c */	extern int lex_order1();
/* setc.c */	extern pset force_lower(pset xlower, pset a, pset b);
/* setc.c */	extern void consensus(pcube r, pcube a, pcube b);
...
/* unate.c */	extern pset_family unate_complement(pset_family A /* disposes of A */);
