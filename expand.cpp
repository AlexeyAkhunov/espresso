...
pcover expand(pcover F, pcover R, bool nonsparse  /* expand non-sparse variables only */)
{
    pcube last, p;
...
#else
 {int w,last;unsigned int x;dist=0;if((last=cube.inword)!=-1)
...
dist+=count_ones(x))>1)goto exit_if;}}}{int w,var,last;pcube
...
void elim_lowering(pcover BB, pcover CC, pcube RAISE, pcube FREESET)
{
	...
	if (! cdist0(p, r))
#else
 {int w,lastw;unsigned int x;if((lastw=cube.inword)!=-1){x=p[
lastw]&r[lastw];if(~(x|x>>1)&cube.inmask)goto false0;for(w=1;w<lastw;w++){x=p[w]
&r[w];if(~(x|x>>1)&DISJOINT)goto false0;}}}{int w,var,lastw;
...
if(p[w]&r[w]&mask[w])goto nextvar;goto false0;nextvar:;}}continue;false0:
#endif
...
{
    pcube p, last, bestfeas = nullptr, *feas;
    int i, j;
     ...
     */
    feas = new pcube[CC->active_count];
    numfeas = 0;
    foreach_active_set(CC, last, p)
	feas[numfeas++] = p;

    /* Setup extra cubes to record parts forced low after a covering */
    feas_new_lower = new pcube[CC->active_count];
