...
void set_pair1(pPLA PLA, bool adjust_labels)
{
    ...
    */
    paired = new bool[cube.num_binary_vars];
    ...
    new_part_size = new int[new_num_vars];
	...
	for(var = 0; var < pair->cnt; var++) {
        ...
        PLA->label[newvar] = strcpy(new char[strlen(scratch)+1], scratch);
	    (void) sprintf(scratch, "%s+%s", var1bar, var2);
	    PLA->label[newvar+1] = strcpy(new char[strlen(scratch)+1], scratch);
	    (void) sprintf(scratch, "%s+%s", var1, var2bar);
	    PLA->label[newvar+2] = strcpy(new char[strlen(scratch)+1], scratch);
	    (void) sprintf(scratch, "%s+%s", var1, var2);
	    PLA->label[newvar+3] = strcpy(new char[strlen(scratch)+1], scratch);
	}
...
{
    bool run;
    int first_run = 0, run_length, var, offset = 0;
...
int **find_pairing_cost(pPLA PLA, int strategy)
{
    ...
    int i, j, xnum_binary_vars = 0, xnum_vars = 0, *xpart_size = nullptr, cost = 0;
    pcover T, Fsave = nullptr, Dsave = nullptr, Rsave = nullptr;
    pset mask;
/*    char *s;*/

    /* data is returned in the cost array */
    cost_array = new int *[cube.num_binary_vars];
    for(i = 0; i < cube.num_binary_vars; i++)
	cost_array[i] = new int[cube.num_binary_vars];
    for(i = 0; i < cube.num_binary_vars; i++)
	for(j = 0; j < cube.num_binary_vars; j++)
	    cost_array[i][j] = 0;
	...
	for(var2 = var1+1; var2 < cube.num_binary_vars; var2++) {
		...
		xpart_size = new int[cube.num_vars];
...
{
    int i, j, besti = 0, bestj = 0, maxcost, total_cost;
    pset cand;
    ppair pair;
...
ppair pair_new(int n)
{
    ...
    pair1 = new pair_t();
    pair1->cnt = 0;
    pair1->var1 = new int[n];
    pair1->var2 = new int[n];
    return pair1;
}
