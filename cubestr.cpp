...
void cube_setup()
{
    ...
    cube.size = 0;
    cube.first_part = new int[cube.num_vars];
    cube.last_part = new int[cube.num_vars];
    cube.first_word = new int[cube.num_vars];
    cube.last_word = new int[cube.num_vars];
	...
	cube.first_word[var] = WHICH_WORD(cube.size);
        cube.size += std::abs(cube.part_size[var]);
	cube.last_part[var] = cube.size - 1;
	cube.last_word[var] = WHICH_WORD(cube.size - 1);
    }

    cube.var_mask = new pset[cube.num_vars];
    cube.sparse = new int[cube.num_vars];
    ...
    cube.temp = new pset[CUBE_TEMP];
    for(i = 0; i < CUBE_TEMP; i++)
	cube.temp[i] = new_cube();
    cube.fullset = set_fill(new_cube(), cube.size);
    cube.emptyset = new_cube();

    cdata.part_zeros = new int[cube.size];
    cdata.var_zeros = new int[cube.num_vars];
    cdata.parts_active = new int[cube.num_vars];
    cdata.is_unate = new int[cube.num_vars];
}

/*
...
void setdown_cube()
{
    ...
    delete cube.first_part;
    delete cube.last_part;
    delete cube.first_word;
    delete cube.last_word;
    delete cube.sparse;
    ...
    delete cube.var_mask;

    for(i = 0; i < CUBE_TEMP; i++)
	free_cube(cube.temp[i]);
    delete cube.temp;

    delete cdata.part_zeros;
    delete cdata.var_zeros;
    delete cdata.parts_active;
    delete cdata.is_unate;
