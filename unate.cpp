...
pcover map_cover_to_unate(pcube * T)
{
    ...
    A = sf_new(static_cast<int>(CUBELISTSIZE(T)), cdata.vars_unate);
    A->count = static_cast<int>(CUBELISTSIZE(T));
    foreachi_set(A, i, p) {
 ...
 */
static pset_family
abs_covered(pset_family A, int pick)
{
    pset last, p, pdest;
    pset_family Aprime;
 ...
 */
static pset_family
abs_covered_many(pset_family A, pset pick_set)
{
    pset last, p, pdest;
    pset_family Aprime;
