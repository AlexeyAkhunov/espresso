...
static void ftautology(pcube * T /* T will be disposed of */, sm_matrix * table);
static bool ftaut_special_cases(pcube * T /* will be disposed if answer is determined */, sm_matrix * table);
...
void
mark_irredundant(pcover F, pcover D)
{
    pcover E, Rt, Rp;
    pset p, p1, last;
    sm_matrix *table;
    ...
    cover = sm_minimum_cover(table, nullptr, /* heuristic */ 1, /* debug */ 0);
...
void
irred_split_cover(pcover F, pcover D, pcover * E, pcover * Rt, pcover * Rp)
{
    pcube p, last;
    int index;
    pcover R;
    ...
    *Rp = new_cover(10);
	...
	if (debug & IRRED1) {
	    (void) printf("IRRED1: zr=%d ze=%d to-go=%d\n",
	...
	if (debug & IRRED1) {
	    (void) printf("IRRED1: zr=%d zrt=%d to-go=%d\n",
		(*Rp)->count, (*Rt)->count,
		R->count - ((*Rp)->count +(*Rt)->count));
	}
    }
    free_cubelist(ED);
...
sm_matrix *
irred_derive_table(pcover D, pcover E, pcover Rp)
{
    pcube last, p, *list;
    sm_matrix *table;
	...
	if (debug & IRRED1) {
	    (void) printf("IRRED1: %d of %d to-go=%d, table=%dx%d\n",
...
bool
cube_is_covered(pcube * T, pcube c)
{
...
bool
tautology(pcube * T /* T will be disposed of */)
{
    pcube cl, cr;
    int best, result;
...
bool
taut_special_cases(pcube * T /* will be disposed if answer is determined */)
{
...
start:
	...
	*Tsave++ = NULL;
	...
	if (debug & TAUT) {
		...
		cdata.vars_unate, static_cast<int>(CUBELISTSIZE(T)));
	}
	goto start;
...
static void
fcube_is_covered(pcube * T, pcube c, sm_matrix * table)
{
...
static void
ftautology(pcube * T /* T will be disposed of */, sm_matrix * table)
{
    pcube cl, cr;
    int best;
...

static bool
ftaut_special_cases(pcube * T /* will be disposed if answer is determined */, sm_matrix * table)
{
