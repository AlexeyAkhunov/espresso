...
sm_alloc()
{
    ...
    A->first_row = A->last_row = nullptr;
    A->first_col = A->last_col = nullptr;
    A->user_word = nullptr;		/* for our user ... */
    return A;
}
...
sm_free(sm_matrix * A)
{
	...
	pcol->first_row = pcol->last_row = nullptr;
	sm_col_free(pcol);
    }
#endif
...
sm_resize(sm_matrix * A, int row, int col)
{
    ...
    if (row >= A->rows_size) {
        new_size = std::max(A->rows_size*2, row+1);
        sm_row ** old = A->rows;
        A->rows = new sm_row*[new_size];
        memcpy(A->rows, old, A->rows_size*sizeof(sm_row*));
        delete old;
    ...
    if (col >= A->cols_size) {
        new_size = std::max(A->cols_size*2, col+1);
        sm_col ** old = A->cols;
        A->cols = new sm_col*[new_size];
        memcpy(A->cols, old, A->cols_size*sizeof(sm_col*));
        delete old;
...
sm_delrow(sm_matrix * A, int i)
{
	...
	prow->first_col = prow->last_col = nullptr;
	sm_row_free(prow);
    }
}

...

int 
sm_read(std::istream& fp, sm_matrix ** A)
{
    int i, j, err;
...
int 
sm_read_compressed(std::istream& fp, sm_matrix ** A)
{
...
void 
sm_write(std::ostream& fp, sm_matrix * A)
{
    sm_row *prow;
    sm_element *p;
    ...
    for(prow = A->first_row; prow != 0; prow = prow->next_row) {
        for(p = prow->first_col; p != 0; p = p->next_col) {
            fp << p->row_num << " " << p->col_num << "\n";
        }
    }
}

void 
sm_print(std::ostream& fp, sm_matrix * A)
{
    sm_row *prow;
    sm_col *pcol;
    int c;
...
{
    (void) printf("%s %d rows by %d cols\n", s, A->nrows, A->ncols);
    if (A->nrows < max) {
        sm_print(std::cout, A);
    }
}
