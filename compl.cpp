...
bool compl_special_cases(pcube * T /* will be disposed if answer is determined */, pcover * Tbar /* returned only if answer determined */);
pcover compl_merge(
                   pcube *T1,			/* Original ON-set */
                   pcover L, pcover R,		/* Complement from each recursion branch */
                   pcube cl, pcube cr,		/* cubes used for cofactoring */
                   int var,			/* splitting variable */
                   int lifting			/* whether to perform lifting or not */);
void compl_d1merge(pcube * L1, pcube * R1);
pcover compl_cube(pcube p);
void compl_lift(pcube * A1, pcube * B1, pcube bcube, int var);
void compl_lift_onset(pcube * A1, pcover T, pcube bcube, int var);
...
bool simplify_special_cases(pcube *T /* will be disposed if answer is determined */, pcover *Tnew /* returned only if answer determined */);
...
bool compl_special_cases(pcube * T /* will be disposed if answer is determined */, pcover * Tbar /* returned only if answer determined */)
{
	...
	*Tbar = compl_cube(set_or(cof, cof, T[2]));
    ...
    size_t size = SET_SIZE(NELEM(cof));
    //std::cerr << "Size: " << size << std::endl;
    ceil = set_copy(new unsigned int[size], cof);
    //ceil = set_save(cof);
    for(T1 = T+2; (p = *T1++) != NULL; ) {
...
void simp_comp(pcube * T /* T will be disposed of */, pcover * Tnew, pcover * Tbar)
{
    pcube cl, cr;
    int best;
...
pcover simplify(pcube * T /* T will be disposed of */)
{
    pcube cl, cr;
    int best;
    pcover Tbar, Tl, Tr;
