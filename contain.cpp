...
*/
pset_family sf_ind_contain(pset_family A /* disposes of A */, int * row_indices /* updated with the new values */)
{
    int cnt;
    pset *A1;
    pset_family R;
...
*/
pset_family sf_union(pset_family A, pset_family B /* disposes of A and B */)
{
    int cnt;
    pset_family R;
    pset *A1 = sf_list(A), *B1 = sf_list(B), *E1;

    E1 = new pset[std::max(A->count, B->count) + 1];
...
*/
pset_family dist_merge(pset_family A /* disposes of A */, pset mask /* defines variables to mask out */)
{
    pset *A1;
    int cnt;
    pset_family R;
...
*/
pset_family d1merge(pset_family A /* disposes of A */, int var)
{
...
int d1_rm_equal(pset *A1 /* array of set pointers */, int (*compare)(const void*, const void*) /* comparison function */)
{
    int i, j, dest;
...
int rm_equal(pset * A1 /* updated in place */, int (*compare)(const void*, const void*))
{
    pset *p, *pdest = A1;
	...
	*pdest = NULL;
    ...
    return static_cast<int>(pdest - A1);
}
...
int rm2_equal(pset * A1, pset * B1 /* updated in place */, pset * E1, int (*compare)(const void*, const void*))
{
    pset *pda = A1, *pdb = B1, *pde = E1;
    ...
    *pda = *pdb = *pde = NULL;
    ...
    return static_cast<int>(pde - E1);
}


/* rm2_contain -- perform containment between two arrays of set pointers */
int rm2_contain(pset * A1 /* updated in place */, pset * B1 /* unchanged */)
{
    pset *pa, *pb, a, b, *pdest = A1;
    ...
    *pdest = NULL;                      /* sentinel */
    return static_cast<int>(pdest - A1);                  /* # elements in A1 */
}
...
pset_family sf_ind_unlist(pset * A1, int totcnt, int size, int * row_indices, pset pfirst)
{
    pset pr, p, *pa;
    int i, *new_row_indices;
    pset_family R = sf_new(totcnt, size);

    R->count = totcnt;
    new_row_indices = new int[totcnt];
...
pset_family sf_merge(pset * A1, pset * B1, pset * E1 /* will be disposed of */, int totcnt, int size)
{
    pset pr, ps, *pmin, *pmid, *pmax;
    pset_family R;
    pset *temp[3], *swap;
    int i, j, n;
