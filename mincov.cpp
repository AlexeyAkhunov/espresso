...
extern int select_column(sm_matrix * A, int * weight, solution_t * indep);
extern void select_essential(sm_matrix * A, solution_t * select, int * weight, int bound /* must beat this solution */);
extern int verify_cover(sm_matrix * A, sm_row * cover);
...
sm_row *
sm_minimum_cover(sm_matrix * A, int * weight,
...
sm_mincov(sm_matrix * A, solution_t * select, int * weight, int lb, int bound, int depth, stats_t * stats)
{
...
#ifdef USE_GIMPEL
    if ( weight == nullptr) {	/* hack until we fix it */
    ...
    lb_new = std::max(select->cost + indep->cost, lb);
    ...
    if (debug) {
        ...
        (void) printf(" %3dx%3d sel=%3d bnd=%3d lb=%3d ",
	...
	if (stats->debug && stats->component == 0) {
            (void) printf("new 'best' solution %d at level %d\n",
	...
	if (best1 != nullptr && bound > best1->cost) {
	    bound = best1->cost;
	}
	...
	if (best1 != nullptr && best1->cost == lb_new) {
	    return best1;
	}
...

void
select_essential(sm_matrix * A, solution_t * select, int * weight, int bound /* must beat this solution */)
{
    sm_element *p;
    sm_row *prow, *essen;
