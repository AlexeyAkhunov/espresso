...
pset sf_or(pset_family A)
{
    ...
    or1 = set_new(A->sf_size);
    foreach_set(A, last, p)
	INLINEset_or(or1, or1, p);
    return or1;
}
...
pset sf_and(pset_family A)
{
    ...
    and1 = set_fill(set_new(A->sf_size), A->sf_size);
    foreach_set(A, last, p)
	INLINEset_and(and1, and1, p);
    return and1;
}
...
pset_family sf_append(pset_family A, pset_family B)
{
    ...
    const unsigned int* old = A->data;
    A->data = new unsigned int[(long) A->capacity * A->wsize];
    memcpy(A->data, old, asize * sizeof(unsigned int));
    delete old;
...
pset_family sf_addset(pset_family A, pset s)
{
    ...
    const unsigned int* old = A->data;
    A->data = new unsigned int[(long) A->capacity * A->wsize];
    memcpy(A->data, old, A->count * A->wsize * sizeof(unsigned int));
    delete old;
    }
    p = GETSET(A, A->count++);
    INLINEset_copy(p, s);
...
void sf_write(std::ostream& fp, pset_family A)
{
    pset p, last;
    fp << A->count << " " << A->sf_size << "\n";
    foreach_set(A, last, p)
	set_write(fp, p);
    fp.flush();
}
...
pset_family sf_read(std::istream& fp)
{
    int i, j;
    pset p, last;
    pset_family A;
...
void set_write(std::ostream& fp, pset a)
{
    int n = LOOP(a), j;
...
pset_family sf_bm_read(std::istream& fp)
{
    int i, j, rows, cols;
    pset pdest;
    pset_family A;
...
void
set_adjcnt(pset a, int * count, int weight)
{
    int i, base;
    unsigned int val;
 ...
 */
pset_family sf_compress(pset_family A /* will be freed */, pset c)
{
    pset p;
    int i, bcol;
    pset_family B;
